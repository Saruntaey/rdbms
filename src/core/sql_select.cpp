...
bool parse_variable_name(qep *q, std::string var_name) {
  BPluskey_t key;
  catalog_rec *cat_rec;
  schema_rec *schm_rec;
  const char *dot_ptr;
  ...
  if (q->col_alias->find(var_name) != q->col_alias->end())
    return true;
  if (q->table_map->find(var_name) != q->table_map->end())
    return true;
  ...
  if (dot_ptr = strchr(var_name.c_str(), '.'); dot_ptr) {
    ...
    *col_name = '\0';
    ...
    q->table_map->insert({var_name, (data_src){.tableIdx = j,
...
bool init_select(qep *q, get_val_fn get_val) {
  ...
  for (i = 0; i < q->select.n; i++) {
    ...
    col->computed_row = -1;
    for (auto it = col->tree->tree->vars.begin();
         it != col->tree->tree->vars.end(); it++) {
      if (!parse_variable_name(q, (*it)->get_name())) {
        printf("fail to parse variable %s \n", (*it)->get_name().c_str());
        return false;
      }
    }
  }
...
bool init_where(qep *q, get_val_fn get_val) {
  std::string var_name;

  if (!q->where.tree)
    return true;
  q->where.tree->tree->setGetVal(get_val);
  for (auto it = q->where.tree->tree->vars.begin();
       it != q->where.tree->tree->vars.end(); it++) {
    var_name = (*it)->get_name();
    if (!parse_variable_name(q, var_name)) {
      printf("init_where: fail to parse variable %s\n", var_name.c_str());
      return false;
    }
  }
  return true;
...
bool init_qep(qep *q) {
  ...
  if (!init_where(q, get_val)) {
    printf("fail to init where\n");
    return false;
  }
