...
static MathExprTree *create_math_exp_tree(parser p);
...
sql_predicate_exp_tree *create_sql_predicate_exp_tree() {
	sql_predicate_exp_tree *t;
	MathExprTree *mt;

	mt = create_math_exp_tree(&S);
	if (!mt) {
		mt = create_math_exp_tree(&Q);
	}
	if (!mt) return nullptr;
	t = (sql_predicate_exp_tree *) calloc(1, sizeof(*t));
	t->tree = mt;
	return t;
}
...
MathExprTree *create_math_exp_tree(parser p) {
	MathExprTree *tree;
	int exp_start;
	parse_status s;
	int postfix_size;
